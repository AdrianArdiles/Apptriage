...
import { signOut, signInWithGoogle, signIn, signUp, GOOGLE_WEB_CLIENT_ID } from "@/lib/firebase-auth";
import {
  Dialog,
  DialogContent,
  DialogDescription,
...
function LandingPage(): React.ReactElement {
  ...
  const [registroMode, setRegistroMode] = React.useState(false);
  ...
  const googleConfigOk = clientId && clientId !== "TU_ID_WEB_AQUÍ" && clientId.endsWith(".apps.googleusercontent.com");

  const handleGoogleSignIn = async () => {
    setLoginError("");
    if (!googleConfigOk) {
      const msg = "Configuración de Google incompleta. Agregá NEXT_PUBLIC_GOOGLE_WEB_CLIENT_ID en .env.local con el ID de cliente web de Firebase (Authentication → Google → ID de cliente web).";
      setLoginError(msg);
    ...
    try {
      if (Capacitor.isNativePlatform()) {
        ...
        if (typeof GoogleAuth.initialize === "function") {
          await GoogleAuth.initialize({ clientId } as Parameters<typeof GoogleAuth.initialize>[0]);
        }
      }
            ...
            {authMode === "google" ? (
              <>
                <button
                  ...
                  disabled={authLoading || !googleConfigOk}
                ...
                {!googleConfigOk && (
                  <p className="text-center text-xs text-amber-300">
                    Configurá NEXT_PUBLIC_GOOGLE_WEB_CLIENT_ID en .env.local para usar Google. Ver documentación.
                  </p>
                )}
                <div className="relative my-4">
                  <span className="absolute inset-0 flex items-center"><span className="w-full border-t border-slate-600" /></span>
                  <span className="relative flex justify-center text-xs font-medium uppercase text-slate-500">o</span>
                </div>
                <button
                  type="button"
              ...
              <form onSubmit={handleEmailAuth} className="space-y-3">
                <div>
                  ...
                  <input
                    ...
                    className="w-full min-h-[48px] rounded-xl border-2 border-slate-600 bg-slate-800 px-4 py-2.5 text-sm text-slate-100 placeholder:text-slate-500 focus:border-blue-500 focus:outline-none"
                  />
                </div>
                <div>
                  ...
                  <input
                    id="login-password"
                    type="password"
                    autoComplete={registroMode ? "new-password" : "current-password"}
                    value={password}
                ...
                <button
                  ...
                  className="w-full min-h-[48px] rounded-xl px-4 py-2.5 text-sm font-semibold text-white transition disabled:opacity-60"
                  style={{ backgroundColor: BLUE_MEDICAL }}
                >
                  {authLoading ? "Esperá…" : registroMode ? "Registrarse" : "Iniciar sesión"}
                </button>
                <button
                  type="button"
                ...
                <button
                  type="button"
                  onClick={() => { setAuthMode("google"); setLoginError(""); setRegistroMode(false); }}
