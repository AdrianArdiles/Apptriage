...
exports.evaluarNivelGravedad = evaluarNivelGravedad;
exports.generarRecomendacion = generarRecomendacion;
function evaluarNivelGravedad(sintomasTexto, signosVitales) {
    const texto = sintomasTexto.toLowerCase();
    let nivel = 2;
    if (signosVitales) {
        if ((signosVitales.saturacionOxigeno !== undefined && signosVitales.saturacionOxigeno < 92) ||
        ...
        else if ((signosVitales.temperatura !== undefined && signosVitales.temperatura >= 39) ||
            (signosVitales.frecuenciaRespiratoria !== undefined && signosVitales.frecuenciaRespiratoria > 24)) {
            nivel = 3;
        }
    }
    if (/dolor (pecho|torácico)|no respiro|desmayo|convulsión|hemorragia|accidente|caída fuerte|inconsciente/i.test(texto)) {
        nivel = Math.max(nivel, 5);
    }
    else if (/fiebre alta|dificultad para respirar|vómito persistente|dolor fuerte|sangre/i.test(texto)) {
    ...
    return Math.min(5, Math.max(1, nivel));
}
